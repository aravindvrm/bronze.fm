...
export const atonomosDocument: DocBlock[] = [
  ...
  },
  {
    "kind": "ul",
    "items": [
      "Dedicated, non-human identities. Every agent gets its own identity and credentials: never a human’s login and never a shared service account borrowed across multiple agents. Borrowing credentials is the path of least resistance during a pilot and the path to the worst finding during an audit.",
      "Short-lived, scoped access over standing permissions. Static, long-lived API keys are exactly the failure mode the identity community is now racing to retire. Agents should authenticate with short-lived, narrowly scoped credentials, provisioned just-in-time for the task at hand, so a compromised or misbehaving agent has a small and time-limited blast radius rather than standing access to everything it might ever need.",
      "A named human owner for every agent. Every agent should be assigned to a manager the way a human employee would be, accountable for its performance, its scope, and its behavior. Critically, that manager governs the agent; they should not hold or be able to use the agent’s own access keys, roles, or permissions. The roles of governing an identity and being that identity must remain separate. Otherwise, the accountability on which the entire model depends breaks down.",
      "Continuous, not periodic, review. Quarterly access reviews were built for humans whose roles rarely change. An agent’s effective permissions and behavior can drift week to week, so the highest-risk agents need automated, near-continuous review rather than a calendar-driven checklist.",
      "A living registry, not tribal knowledge. The enterprise needs a single, current inventory of every agent in operation, including what it can access, who owns it, and what it has done. That way, “which agents do we actually have running?” is always an answerable question, not an open investigation."
    ]
  },
  {
    "kind": "p",
  ...
  },
  {
    "kind": "ul",
    "items": [
      "Agent Identity Registry: Stand up a central, mandatory registry for every agent deployed anywhere in the enterprise: identity, owner, scope, credentials, and status. No agent goes into production without an entry.",
      "Non-Human IAM Platform: Extend or replace legacy IAM with a platform purpose-built for machine and agent identities that supports short-lived credentials, delegation claims, and per-action scoping instead of static keys and broad role assignments.",
      "Ownership and Accountability Model: Require a named human owner for every agent, distinct from whoever administers its credentials. Make that ownership a factor in performance reviews the same way owning a system or a process would be.",
      "Kill Switches and Revocation: Ensure every agent’s access can be suspended immediately and independently of other agents sharing its role or team, without waiting on a broader system change.",
      "Alignment to Emerging Standards: Track the NIST AI RMF, NIST SP 800-207, the CSA Agentic Trust Framework, and OWASP’s agentic and non-human identity guidance, and architect toward them now rather than retrofitting later once they harden into requirements."
    ]
  },
  {
    "kind": "h",
  ...
  {
    "kind": "p",
    "text": "Objective Definition Frameworks: Develop standard ways to specify goals and tasks for AI agents (e.g. structured requests, process templates, or natural language instructions that agents can parse). Ensure goals are tied to business KPIs so agents optimize for true outcomes, not proxies."
  },
  {
    "kind": "p",
    "text": "Policy & Constraint Libraries: Encode organization policies, regulatory requirements, and business rules in a central repository (using policy-as-code tools or rule engines). For example, define approval thresholds, data access rules, and compliance checks in machine-readable form. Agents should query this library before acting."
  },
  {
    "kind": "p",
    "text": "Governance Boards: Establish an AI governance committee or role (e.g. AI Policy Officer) responsible for updating and reviewing the goals/constraints given to agents. This group ensures that as business strategy or regulations change, the agent directives are adjusted accordingly."
  },
  {
    "kind": "p",
    "text": "Human Oversight Mechanisms: Implement dashboards and alerts for human supervisors to monitor agent activities. Set up review checkpoints (either time-based or event-based) where humans validate that the outcomes agents produce indeed align with the high-level intent. This might include sampling agent decisions for quality assurance, much like a manager reviews a subordinate’s work."
  },
  {
    "kind": "h",
    "level": 3,
  ...
  {
    "kind": "p",
    "text": "Automation Inventory: Create an enterprise-wide inventory of processes and tasks, identifying which can be executed by AI agents. Start with high-volume, routine processes to maximize impact.Digital Workforce Training: Invest in platforms to train and deploy AI agents for specific roles. Ensure each agent’s scope and abilities are well-defined.Exception Handling Protocols: Define clear rules for when agents must defer to humans – e.g. approval requirements, novel situations, or ethical judgments. Implement an escalation system so agents automatically hand off such cases with context.Change Management: Communicate the shift to all staff, emphasizing that agents free them from grunt work rather than threaten their jobs. Provide training for employees to effectively supervise and collaborate with AI agents."
  },
  {
    "kind": "h",
    "level": 3,
  ...
  {
    "kind": "p",
    "text": "Enterprise API Inventory: Catalog all existing APIs and identify gaps where critical functions lack programmatic access. For each major system (CRM, finance, etc.), ensure there is a strategy to expose needed actions via API or automation scripts."
  },
  {
    "kind": "p",
    "text": "Legacy System Wrapping: For legacy or third-party systems without APIs, consider RPA wrappers or middleware that expose a pseudo-API. For instance, use RPA bots to interact with a legacy UI but provide a REST API for agents to trigger those bots."
  },
  {
    "kind": "p",
    "text": "Developer Platform & Standards: Create internal standards and tools for API development (consistent authentication, logging, error handling). Provide a self-service portal where teams can discover and request access to APIs and data for their agents."
  },
  {
    "kind": "p",
    "text": "Microservices and Modularization: Encourage breaking monolithic applications into microservices or at least adding a service layer. Modular business logic so that even internal processes are callable services that agents could leverage ."
  },
  {
    "kind": "p",
    "text": "Event-Driven Architecture: Implement an event bus or messaging system to broadcast important business events. Ensure agents are designed to subscribe to relevant events (e.g. an “OrderPlaced” event triggers an order-fulfillment agent workflow)."
  },
  {
    "kind": "p",
    "text": "API Governance: Monitor usage of APIs by agents – track performance, failures, and security. Apply rate limiting or concurrency controls to prevent runaway agents from overwhelming services. Include APIs and integrations in disaster recovery planning (agents should gracefully handle when a service is unavailable)."
  },
  {
    "kind": "h",
    "level": 3,
  ...
  {
    "kind": "p",
    "text": "Concretely, this means instrumenting agents and processes such that every significant event is logged: what decision was made, what data was accessed, which API was called, what outcome occurred, etc. Each agent action should be timestamped and tied to a “reason” or context. This could include storing the input prompt and output of an AI model for later review. The enterprise should be able to reconstruct the sequence of steps an agent took on any given transaction – in other words, achieve end-to-end traceability. In an agent-native environment, nothing important should happen in a black box. If an agent executes a trade or changes a customer record, stakeholders should be able to inspect that action after the fact (or even in real-time) with as much clarity as if a human had done it – including understanding why it happened."
  },
  {
    "kind": "p",
  ...
  {
    "kind": "p",
    "text": "Unified Logging Framework: Adopt or build a logging framework that all agents use. Ensure logs capture inputs, outputs, decisions, and any rule/ML model influences on each decision. Structure logs in a way that they can be queried for audit (e.g. JSON logs with keys for action type, outcome, etc.)."
  },
  {
    "kind": "p",
    "text": "Audit Trail Systems: Integrate agent logs with existing audit/compliance systems. For instance, if you have an audit requirement for financial transactions, ensure the agent’s actions on those transactions are recorded in the same audit repository. Use immutability techniques (write-once storage or blockchain-style ledger if appropriate) to prevent tampering."
  },
  {
    "kind": "p",
    "text": "Real-Time Monitoring: Implement dashboards that show key metrics of agent operations (throughput, success rate, error rate, queue lengths, etc.). Set up alerts – for example, if an agent starts erroring out on more than 5% of tasks in a hour, page an operator."
  },
  {
    "kind": "p",
    "text": "Traceability and Explainability: Where applicable, log the “why” behind decisions. For rule-based agents, this could be the rule ID or condition that was true. For learning-based agents, consider storing model version and any confidence scores or attention weights. Use tools that can analyze model decisions (like LLM prompt+response logs) to provide post-hoc explanations."
  },
  {
    "kind": "p",
    "text": "Ownership and Governance: Assign each AI agent an owner (a human product/process owner responsible for it) . That owner should regularly review the agent’s logs and performance. Additionally, set up a cadence (e.g. monthly AI audit meetings) where various stakeholders (IT, compliance, business line) review agent operation reports – similar to how financials are reviewed."
  },
  {
    "kind": "p",
    "text": "Built-in Reversibility: Design processes so that any agent action can be rolled back if found erroneous. This may involve keeping checkpoints or designing transactions to be compensatable. As noted in one principle, “approvals, audit trails, and rollback should be native behaviors” of the automation platform . If an agent mistakenly updates data, there should be an easy way to revert or correct it (possibly automated or via a human-triggered action)."
  },
  {
    "kind": "h",
    "level": 3,
  ...
  {
    "kind": "p",
    "text": "Declarative Policies (“Policy as Code”): This involves writing policies in code-like formats (using languages or engines like Open Policy Agent’s Rego, or AWS Config rules, etc.). For access control, deployment rules, compliance checks – treat them like unit tests or if-then logic that can be evaluated by machines continuously. One example in AI operations: codifying escalation paths and approval matrices such that an agent can automatically know who to ask (and how) when a certain condition is met, rather than relying on tribal knowledge. A transparent governance model with policy-as-code and case memory makes actions explainable and auditable, as noted earlier ."
  },
  {
    "kind": "p",
    "text": "Executable Contracts and Documents: In some domains, contracts or requirements can be made executable. For instance, Service Level Agreements (SLAs) could be tied to monitors that automatically trigger agents if conditions aren’t met. Or take the concept of “smart contracts” in blockchain – similarly, an agent-native enterprise might express inter-department agreements in a form that agents can use to coordinate work (e.g. if Sales closes a deal, an agent automatically triggers provisioning per the agreed timeline, because the contract is digitized in the system)."
  },
  {
    "kind": "p",
    "text": "Organizationally, moving to machine-readable processes requires a concerted effort to capture tacit knowledge and formalize it. Many organizations have a wealth of know-how in employees’ heads or scattered documents. Initiatives like knowledge graphs or process mining can help extract this and encode it. For example, observing how employees handle cases and then turning that into a structured process model is one way (some organizations like Skan specialize in this “Observation-to-Agent” approach ). It may be useful to form a dedicated Process Automation Team that works with business units to translate their SOPs and rules into machine-executable formats. Over time, as new processes are created, the default should be to create them in a digital, structured format first, and only then derive any textual documentation from that – essentially treating the machine-readable version as the source of truth."
  },
  {
    "kind": "p",
    "text": "The benefits of machine-first process definition are tremendous: it ensures that AI agents have a clear blueprint to follow and reduces errors from misinterpretation. It also accelerates automation – if your processes are already in an AI-readable form, deploying a new agent to handle a process is much faster. One case study in helpdesk operations found that by encoding SOPs into a structured format, they could automatically resolve routine tasks and allow human agents to focus on complex cases, significantly improving resolution times and consistency . Additionally, having processes as code means you can simulate and test them. Before deploying an agent, you might run the process logic against historical data to see if it yields the expected outcomes – akin to unit-testing your business operations."
  },
  {
    "kind": "p",
  ...
  {
    "kind": "p",
    "text": "Process Modeling Tools: Invest in tools that allow non-programmers to define workflows and rules in a structured way (low-code platforms, visual BPMN modelers, etc.). Ensure these tools can output an executable process that agents or automation engines can run."
  },
  {
    "kind": "p",
    "text": "Knowledge Capture: Before automating a process, capture it in a formal model. Leverage process mining tools that watch user interactions to suggest process models. Interview domain experts and translate their step-by-step approach into decision trees or pseudocode that can be coded."
  },
  {
    "kind": "p",
    "text": "Policy-as-Code Platforms: Use or build a central policy engine for things like approvals, business rules, compliance checks. For example, use Open Policy Agent or similar to write policies in a declarative manner that all services and agents can query. Integrate this engine with your orchestration layer so that policies are enforced consistently by agents during execution ."
  },
  {
    "kind": "p",
    "text": "Templates and Reuse: Create templates for common patterns (e.g. an “approvals workflow” template, a “data validation” step, etc.) so that process authors can easily assemble new processes from proven building blocks. This encourages consistency across the enterprise."
  },
  {
    "kind": "p",
    "text": "Human-Friendly Views: While machine-readable is primary, provide generated human-readable documentation from the source (for compliance and training purposes). For instance, automatically generate flow diagrams or English descriptions from the BPMN or code so that stakeholders can review the logic. This fosters transparency and trust – everyone can see what the agents will do in a format comfortable to them."
  },
  {
    "kind": "p",
    "text": "Testing and Simulation: Treat processes as you would software: do QA on them. Simulate processes with test data to ensure the machine-readable SOP does what the human-intended SOP was. When possible, run processes in parallel (agent vs human) on sample cases before full deployment to verify parity."
  },
  {
    "kind": "h",
    "level": 2,
  ...
  {
    "kind": "p",
    "text": "The journey to an agent-native enterprise is unquestionably challenging – it touches every facet of the business from IT architecture to org chart to daily routines. Yet, the reward is transformational. By making AI agents first-class citizens, enterprises unlock speed, scale, and intelligence that far exceed what traditional operations can achieve. Imagine business processes that execute in seconds what used to take days, services that run 24/7 without fatigue, and the ability to adapt on the fly as data and conditions change. Organizations that have pioneered this approach are already reaping benefits: higher operational throughput, dramatically lower error rates, improved customer experiences through consistency, and employees liberated to focus on innovation over administration . Surveys show over 65% of professionals believe AI will significantly enhance or revolutionize automation – and “the future belongs to the organizations prepared to harness this powerful combination” of AI and enterprise operations ."
  },
  {
    "kind": "p",
    "text": "For enterprise leaders, the mandate is clear. The true “agent-native” enterprise is not achieved by half-measures or bolt-on solutions. It requires a holistic redesign embracing the five principles we outlined: maximizing agent automation, redefining human roles to strategic level, building API-centric ecosystems, enforcing observability and trust, and formalizing knowledge into machine-executable form. Start laying the groundwork today:"
  },
  {
    "kind": "p",
