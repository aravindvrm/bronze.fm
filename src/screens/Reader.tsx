...
function stripTitlePage(blocks: DocBlock[], title: string): DocBlock[] {
  ...
  while (
    ...
    heading.includes(
      normalise((blocks[i] as Extract<DocBlock, { kind: "h" }>).text),
    )
  ) {
    i++;
  }
...
function Run({ span }: { span: Span }) {
  const classes = [
    ...
    span.em && "italic",
    span.code &&
      "rounded bg-parchment/[0.07] px-1 py-0.5 font-mono text-[0.85em]",
  ]
    .filter(Boolean)
    .join(" ");
  ...
  switch (block.kind) {
    ...
    case "ul":
      return (
        <ul
          data-block={index}
          className="mt-3 list-disc space-y-2 pl-5 marker:text-gilt/50"
        >
...
export function Reader() {
  ...
  const { page, pages, blockPages, anchorBlock, goToPage, goToBlock, turn } =
    usePagination(outerRef, innerRef, [blocks, scaleIndex, hasQueue]);
  ...
  useEffect(() => {
    if (restored.current || !content || pages <= 1 || blockPages.length === 0)
      return;
  ...
  const minutesLeft =
    pages > 1 ? Math.ceil((totalWords * (1 - page / (pages - 1))) / WPM) : 0;
    ...
    return blocks
      ...
      .map(({ i, text }) => {
        ...
        return {
          ...
          excerpt:
            (from > 0 ? "…" : "") +
            text.slice(from, at + q.length + 60).trim() +
            "…",
        };
      });
  ...
  return (
     ...
     */
    <div
      className="fixed inset-x-0 top-0 z-30 flex flex-col bg-void"
      style={{ height: "100dvh" }}
    >
      {/*
      ...
      */}
      ...
      >
        ...
        >
          ...
          >
            <h1 className="mb-6 text-[2em] font-semibold leading-tight text-parchment">
              {title}
            </h1>
            {blocks.map((block, i) => (
